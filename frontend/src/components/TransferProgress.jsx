...
import { getTransfers, getTransferChecksums } from '../services/api'

/**
 * TransferProgress Component
 * Shows active/in-progress transfers with real-time updates
 *
 * MRC: Simple polling-based updates every 2 seconds
 * Week 5: Shows folder and watch mode badges
 */
function TransferProgress() {
  ...
  useEffect(() => {
    loadTransfers()
    const interval = setInterval(loadTransfers, 2000) // Poll every 2s
  ...
  async function loadTransfers() {
    try {
      ...
      // Filter for active transfers (not completed/failed)
        ...
        <div key={transfer.id} className="transfer-card active">
          ...
          <div className="transfer-actions">
            <button
              ...
              onClick={() => showChecksums(transfer.id)}
            >
              View Checksums
            </button>
          </div>
        ...
        <div className="checksum-list">
          {checksums.items.map(checksum => (
            <div key={checksum.id} className="checksum-item">
              <div className="checksum-type">
                {checksum.checksum_type.toUpperCase()}
              </div>
              <div className="checksum-value">
                {checksum.checksum_value}
              </div>
              <div className="checksum-meta">
                Calculated in {checksum.calculation_duration_seconds}s
              </div>
            </div>
          ))}
        </div>
